<template>
  <div class="app-container" id="systemSettingsAdminList">
    <pageName></pageName>
    <el-button type="primary" class="add-admin" @click="addAdmin">+ 添加管理员</el-button>

    <div class="divider"></div>

    <el-table
      :data="tableData"
      border
      header-cell-class-name="table-header-cell-lyw"
      row-class-name="table-column-lyw"
      style="width: 100%">
      <el-table-column
        prop="id"
        align="center"
        label="ID"
        width="329">
      </el-table-column>
      <el-table-column
        prop="account"
        align="center"
        label="账号"
        width="329">
      </el-table-column>
      <el-table-column
        prop="group"
        align="center"
        label="所属分组"
        width="329">
      </el-table-column>
      <el-table-column
        prop="createDate"
        align="center"
        label="添加时间"
        width="329">
      </el-table-column>
      <el-table-column
        prop="tag"
        align="center"
        label="操作">
        <template slot-scope="scope">
          <div class="icons">
            <img @click="edit(scope)" src="../../assets/images/bianji.png" style="margin-right: 55px;">
            <img @click="openDialog(scope)" src="../../assets/images/shanchu.png">
          </div>
        </template>
      </el-table-column>
    </el-table>

    <div class="ctrl">
      <div style="margin-right: 20px;">共计15个记录分为1页，当前第 1 页</div>
      <el-button class="btn-ctrl">第一页</el-button>
      <el-button class="btn-ctrl">上一页</el-button>
      <el-button class="btn-ctrl">下一页</el-button>
      <el-button class="btn-ctrl">最末页</el-button>
      <div style="margin: 0 18px 0 22px;">跳转到</div>
      <el-input v-model="pageNum" placeholder="0" style="width: 50px;"></el-input>
      <el-button type="primary" class="btn-ctrl" style="color: #FFF;margin-left: 10px;">确认</el-button>
    </div>

    <dialogLyw ref="dialogLyw">
      <div slot="header">删除之后将不能恢复，是否确认删除?</div>
    </dialogLyw>

    <dialogLyw ref="dialogEditLyw" height="500px" width="610px" :footerStyle="{left: '35px'}">
      <div slot="header">编辑管理员?</div>
      <div slot="body">

        <div class="list">
          <div class="txt">账号</div>
          <el-input placeholder="输入查找内容" style="width: 350px;"></el-input>
        </div>

        <div class="list">
          <div class="txt">密码</div>
          <el-input placeholder="输入查找内容" style="width: 350px;"></el-input>
        </div>

        <div class="list">
          <div class="txt">所属组</div>
          <el-select v-model="value" placeholder="请选择" style="width: 350px;">
            <el-option
              v-for="item in options"
              :key="item.value"
              :label="item.label"
              :value="item.value">
            </el-option>
          </el-select>
        </div>

      </div>
    </dialogLyw>

  </div>
</template>

<script>

const tableData = [{
  createDate: '2016-05-02 08:30:00',
  account: '王小虎1王小虎1王小虎1',
  id: '1',
  group: '订单组',
  address: '上海市普陀区金沙江路 1518 弄'
}, {
  createDate: '2016-05-04 08:30:00',
  account: '王小虎2王小虎2王小虎2',
  id: '2',
  group: '订单组',
  address: '上海市普陀区金沙江路 1517 弄'
}, {
  createDate: '2016-05-01 08:30:00',
  account: '王小虎3王小虎3王小虎3',
  id: '3',
  group: '订单组',
  address: '上海市普陀区金沙江路 1519 弄'
}, {
  createDate: '2016-05-03 08:30:00',
  account: '王小虎4王小虎4王小虎4',
  id: '4',
  group: '订单组',
  address: '上海市普陀区金沙江路 1516 弄'
}, {
  createDate: '2016-05-03 08:30:00',
  account: '王小虎4王小虎4王小虎4',
  id: '5',
  group: '订单组',
  address: '上海市普陀区金沙江路 1516 弄'
}, {
  createDate: '2016-05-03 08:30:00',
  account: '王小虎4王小虎4王小虎4',
  id: '6',
  group: '订单组',
  address: '上海市普陀区金沙江路 1516 弄'
}, {
  createDate: '2016-05-03 08:30:00',
  account: '王小虎4王小虎4王小虎4',
  id: '7',
  group: '订单组',
  address: '上海市普陀区金沙江路 1516 弄'
}, {
  createDate: '2016-05-03 08:30:00',
  account: '王小虎4王小虎4王小虎4',
  id: '8',
  group: '订单组',
  address: '上海市普陀区金沙江路 1516 弄'
}, {
  createDate: '2016-05-03 08:30:00',
  account: '王小虎4王小虎4王小虎4',
  id: '9',
  group: '订单组',
  address: '上海市普陀区金沙江路 1516 弄'
}, {
  createDate: '2016-05-03 08:30:00',
  account: '王小虎4王小虎4王小虎4',
  id: '10',
  group: '订单组',
  address: '上海市普陀区金沙江路 1516 弄'
}]

import pageName from '@/components/pageName'
import dialogLyw from '@/components/dialogLyw'

// import module from './module'
// Js部分尽量采用ES6语法，webpack babel插件会转义兼容
export default {
  name: 'systemSettingsAdminList',
  // 要用到哪些子组件（如果组件已是最小粒度，那么可省略该属性）
  components: {
    pageName,
    dialogLyw
  },
  mixins: [],
  // 组件私有数据（必须是function，而且要return对象类型）
  data() {
    return {
      tableData,
      pageNum: 0,
      dialogActiveItem: {},
      options: [{
        value: '选项1',
        label: '黄金糕'
      }, {
        value: '选项2',
        label: '双皮奶'
      }, {
        value: '选项3',
        label: '蚵仔煎'
      }, {
        value: '选项4',
        label: '龙须面'
      }, {
        value: '选项5',
        label: '北京烤鸭'
      }, {
        value: '选项5',
        label: '北京烤鸡'
      }, {
        value: '选项5',
        label: '北京猪'
      }, {
        value: '选项5',
        label: '北京烤鹅'
      }],
      value: ''
    }
  },
  // 父组件传递过来的数据（两种方式声明：1.数组 2.对象）
  props: {

  },
  // 计算属性
  computed: {

  },
  // 监听
  watch: {

  },
  // 函数集，自己封装，便于开发使用
  methods: {
    addAdmin () {
      this.$refs.dialogEditLyw.dialogVisible = true
    },
    openDialog (item) {
      this.dialogActiveItem = item.row
      this.$refs.dialogLyw.dialogVisible = true
    },
    edit (item) {
      this.dialogEditActiveItem = item.row
      this.$refs.dialogEditLyw.dialogVisible = true
    }
  },
  // 生命周期钩子：实例初始化之后，数据观测(data observer) 和 event/watcher 事件配置之前被调用
  beforeCreated() {

  },
  // 生命周期钩子：组件实例完成创建之后调用
  created() {
    this.changePageName('管理员列表')
  },
  // 生命周期钩子：组件实例渲染完成时调用
  mounted() {

  }
}
</script>

<style rel="stylesheet/scss" lang="scss">
@import "src/styles/lyw/index.scss";
#systemSettingsAdminList {
  .add-admin {
    font-size: $fz18;
    margin-top: 23px;
  }

  .divider {
    border-top: 1px solid $borderColor;
    margin: 20px 0 20px 0;
  }

  .icons {
    @include flexBox(flex, center, center);
    img {
      width: 16px;
      height: 16px;
      @include cursorPointer;
    }
  }

  .ctrl {
    margin-top: 60px;
    font-size: $fz14;
    height: 34px;
    border-bottom: 1px solid $borderColor;
    padding-bottom: 20px;
    @include flexBox(flex, flex-end, center);
    .btn-ctrl {
      color: $txtColor2;
    }
    .el-input__inner {
      height: 34px;
    }
  }

  .list {
    display: flex;
    align-items: center;
    margin-bottom: 20px;
    .el-input__inner {
      height: 50px;
    }
    .txt {
      font-size: $fz14;
      text-align: right;
      color: #575757;
      margin-right: 30px;
      width: 60px;
    }
  }
}
</style>
